...
a2 = [ones(m,1) a2']; %(26*5000)  theta2 = 10*26
z3 = Theta2*a2';
...
J = sum(sum(-y_new.*log(h_theta)-(1-y_new).*log(1-h_theta)));
...
t2 = Theta2(:,2:end); %removing both bias so 10*25
...
for j=1:m
    ...
    del_3 = a3 - y_new(:,j); 
    z2 = [1;z2];
    del_2 = (Theta2'*del_3).*sigmoidGradient(z2); %26*1
    del_2 = del_2(2:end);
...
Theta1_grad(:,2:end) = (1/m)*Theta1_grad(:,2:end) + (lambda/m)*(t1);
Theta2_grad(:,2:end) = (1/m)*Theta2_grad(:,2:end) + (lambda/m)*(t2);
